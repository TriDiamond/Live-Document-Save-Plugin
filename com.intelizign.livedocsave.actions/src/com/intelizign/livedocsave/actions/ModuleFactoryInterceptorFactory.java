...
public class ModuleFactoryInterceptorFactory implements ServiceInterceptorFactory {
	private IDataService dataService;

	public ModuleFactoryInterceptorFactory(IDataService dataService) {
	...
	public void createInterceptor(InterceptorStack stack, Module arg1, List arg2) {
		...
		stack.push(new IPObjectFactory() {
			...
			public IPObject createObjectForURI(SubterraURI uri, IDataService dataService) {
				return delegate.createObjectForURI(uri, ModuleFactoryInterceptorFactory.this.dataService);
			}

			@Override
			public IPObject createObjectForDAO(IDataObject dao, IDataService dataService) {
				return delegate.createObjectForDAO(dao, ModuleFactoryInterceptorFactory.this.dataService);
			}

			@Override
			public IPObject createNewObject(IPrototype prototype, IDataService dataService) {
				return delegate.createNewObject(prototype, ModuleFactoryInterceptorFactory.this.dataService);
			}
		});
	}
